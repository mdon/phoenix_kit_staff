...
- Avatar is a single pointer in `Person.metadata["avatar_uuid"]` (no column) via `Attachments.{avatar_uuid, avatar_file, avatar_url, set_avatar, clear_avatar}`; metadata writes merge, never clobber other keys; `set_avatar/2` refuses a trashed person and any file that is not a live image in the person's own `Images` folder (the media tab's "set as avatar" sends a client-side uuid). `PersonShowLive` hosts the picker and logs `staff.person_avatar_set/removed`.
